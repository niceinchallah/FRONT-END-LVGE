...
const Veh = () => {
  ...
  const totalVehiclesWashed = selectedData.reduce((acc, value) => acc + value, 0);
  const money = totalVehiclesWashed*34;
  const adjustedValues = selectedData.map(value => (value / totalVehiclesWashed) * totalVehiclesWashed);
...
`<span>${viewTypeLabel}: ${adjustedValues[dataPointIndex].toFixed(0)} Vehicle</span>`
...
>
  ...
  {`
  ${totalVehiclesWashed} Vehicle = $${money}`}
</Typography>
